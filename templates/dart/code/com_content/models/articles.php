...
class ContentModelArticles extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// override cat id from search
		
		if(isset($_GET['category']) && $_GET['category']) {
			$categoryId = (int)$_GET['category'];
			$query->where('a.catid = ' . $categoryId);
			$this->getState('filter.category_id', $categoryId);
		...
		if(isset($_GET['year']) && $_GET['year']) {
			$time = strtotime($_GET['year']. '-01-01');
			$from = date("Y-m-d H:i:s", $time);
			$to = date("Y-12-31 23:59:59", $time);
			$query->where('a.publish_up > "' . $from . '"');
			$query->where('a.publish_up < "' . $to . '"');
		}
