...
class FaqsModelQuestion extends JModelItem
{
	...
	{
		$app  = JFactory::getApplication('com_faqs');
		...
		{
			$id = JFactory::getApplication()->getUserState('com_faqs.edit.question.id');
		}
		else
		{
			$id = JFactory::getApplication()->input->get('id');
			JFactory::getApplication()->setUserState('com_faqs.edit.question.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new Exception(JText::_('COM_FAQS_ITEM_NOT_LOADED'), 403);
					}
				}
	...
	{
		$this->addTablePath(JPATH_ADMINISTRATOR . '/components/com_faqs/tables');
